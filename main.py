...
def main():
  ...
  if not args.latest:
    ...
    url = "https://api.anilibria.app/api/v1/anime/releases/" + str(results[resultNumber - 1]['id']) + "?"
  ...
  if title['type']['description'] == 'Фильм':
    ...
    while f"{episodeLink}" == "None":
      ...
      episodeLink = episodes[0]['hls_' + args.quality]
    if args.download:
      downloadLink(episodeLink, "MOVIE", f"{title['name']['main']} ({args.quality}).mp4")
    else:
  ...
  #Если нет отрисовывает список эпизодов
    ...
    try:
      ...
      sys.exit()
    if args.download:
      downloadLink(episodeLink, title['name']['main'], f"Серия {args.episode} ({args.quality}).mp4")
    else:
